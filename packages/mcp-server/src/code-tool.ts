...
const prompt = `Runs JavaScript code to interact with the Stainless9 API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ...
    STAINLESS9_BASE_URL:
      readEnv('STAINLESS9_BASE_URL') ?? readEnv('STAINLESS9_ENVIRONMENT') ?
        undefined
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'stainless9',
      code,
      intent,
      client_opts: {
        project: readEnv('STAINLESS9_PROJECT'),
        environment: (readEnv('STAINLESS9_ENVIRONMENT') || undefined) as any,
      },
