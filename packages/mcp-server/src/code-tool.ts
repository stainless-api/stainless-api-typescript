...
const prompt = `Runs JavaScript code to interact with the Stainless4 API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ...
    STAINLESS4_BASE_URL:
      readEnv('STAINLESS4_BASE_URL') ?? readEnv('STAINLESS4_ENVIRONMENT') ?
        undefined
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'stainless4',
      code,
      intent,
      client_opts: {
        project: readEnv('STAINLESS4_PROJECT'),
        environment: (readEnv('STAINLESS4_ENVIRONMENT') || undefined) as any,
      },
