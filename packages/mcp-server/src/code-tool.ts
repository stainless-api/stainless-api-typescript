...
import { Stainless3, ClientOptions } from '@stainless-api/sdk';

const prompt = `Runs JavaScript code to interact with the Stainless3 API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { STAINLESS_API_KEY: readEnv('STAINLESS_API_KEY') ?? client.apiKey ?? undefined, STAINLESS3_BASE_URL: readEnv('STAINLESS3_BASE_URL') ?? readEnv('STAINLESS3_ENVIRONMENT') ? undefined : client.baseURL ?? undefined };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: "stainless3",
      code,
      intent,
      client_opts: { project: readEnv('STAINLESS3_PROJECT'), environment: (readEnv('STAINLESS3_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@stainless-api/sdk';
