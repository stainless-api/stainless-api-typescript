...
import { Stainless2, ClientOptions } from '@stainless-api/sdk';

const prompt = `Runs JavaScript code to interact with the Stainless2 API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { STAINLESS_API_KEY: readEnv('STAINLESS_API_KEY') ?? client.apiKey ?? undefined, STAINLESS2_BASE_URL: readEnv('STAINLESS2_BASE_URL') ?? readEnv('STAINLESS2_ENVIRONMENT') ? undefined : client.baseURL ?? undefined };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: "stainless2",
      code,
      intent,
      client_opts: { project: readEnv('STAINLESS2_PROJECT'), environment: (readEnv('STAINLESS2_ENVIRONMENT') || undefined) as any },
