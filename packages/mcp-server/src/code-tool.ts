...
const prompt = `Runs JavaScript code to interact with the Stainless7 API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ...
    STAINLESS7_BASE_URL:
      readEnv('STAINLESS7_BASE_URL') ?? readEnv('STAINLESS7_ENVIRONMENT') ?
        undefined
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'stainless7',
      code,
      intent,
      client_opts: {
        project: readEnv('STAINLESS7_PROJECT'),
        environment: (readEnv('STAINLESS7_ENVIRONMENT') || undefined) as any,
      },
