...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ...
    STAINLESS14_BASE_URL:
      readEnv('STAINLESS14_BASE_URL') ?? readEnv('STAINLESS14_ENVIRONMENT') ?
        undefined
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'stainless14',
      code,
      intent,
      client_opts: {
        project: readEnv('STAINLESS14_PROJECT'),
        environment: (readEnv('STAINLESS14_ENVIRONMENT') || undefined) as any,
      },
