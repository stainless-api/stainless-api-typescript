...
export async function initMcpServer(params: {
  ...
  let _client: Stainless16 | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): Stainless16 => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new Stainless16({
          ...{
            project: readEnv('STAINLESS16_PROJECT'),
            environment: (readEnv('STAINLESS16_ENVIRONMENT') || undefined) as any,
          },
          logger,
