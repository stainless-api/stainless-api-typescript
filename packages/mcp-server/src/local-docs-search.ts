...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: [
      'project: string;',
      'revision: { files: object; merge: string; } | string | object;',
      'allow_empty?: boolean;',
      'branch?: string;',
      'commit_message?: string;',
  ...
  },
  {
    ...
    params: [
      'base: { branch: string; revision: { files: object; merge: string; } | string | object; commit_message?: string; };',
      'head: { branch: string; revision: { files: object; merge: string; } | string | object; commit_message?: string; };',
      'project: string;',
      'targets?: string[];',
    ],
    response:
    ...
    perLanguage: {
      ...
      ruby: {
        method: 'builds.compare',
        example:
          'require "stainless_v0"\n\nstainless = StainlessV0::Client.new(\n  api_key: "My API Key",\n  environment: "staging" # defaults to "production"\n)\n\nresponse = stainless.builds.compare(\n  base: {branch: "branch", revision: {files: {foo: {content: "content"}}, merge: "merge"}},\n  head: {branch: "branch", revision: {files: {foo: {content: "content"}}, merge: "merge"}},\n  project: "project"\n)\n\nputs(response)',
      },
      cli: {
        method: 'builds compare',
        example:
  ...
  },
  {
    ...
    response:
      "{ output: 'url'; target: string; type: string; url: string; path?: string; } | { token: string; output: 'git'; ref: string; target: string; type: string; url: string; } | { content: string; output: 'content'; target: string; type: string; path?: string; }",
    markdown:
      "## retrieve\n\n`client.builds.targetOutputs.retrieve(build_id: string, target: string, type: string, output?: 'url' | 'git' | 'content', path?: string): { output: 'url'; target: target; type: string; url: string; path?: string; } | { token: string; output: 'git'; ref: string; target: target; type: string; url: string; } | { content: string; output: 'content'; target: target; type: string; path?: string; }`\n\n**get** `/v0/build_target_outputs`\n\nRetrieve a method to download an output for a given build target.\n\nIf the requested type of output is `source`, and the requested output\nmethod is `url`, a download link to a tarball of the source files is\nreturned. If the requested output method is `git`, a Git remote, ref,\nand access token (if necessary) is returned.\n\nOtherwise, the possible types of outputs are specific to the requested\ntarget, and the output method _must_ be `url` or `content`. See the\ndocumentation for `type` for more information.\n\nThe `content` output method returns the raw content of the output inline,\ninstead of a download URL, and is only supported for the `openapi-*` and\n`file` types.\n\n### Parameters\n\n- `build_id: string`\n  Build ID\n\n- `target: string`\n  SDK language target name\n\n- `type: string`\n\n- `output?: 'url' | 'git' | 'content'`\n  Output format: url (download URL), git (temporary access token), or content\n(raw content returned inline, only supported for the \"openapi-*\" and \"file\"\ntypes).\n\n- `path?: string`\n  The path of the file to get when used with \"type\": \"file\".\n\n### Returns\n\n- `{ output: 'url'; target: string; type: string; url: string; path?: string; } | { token: string; output: 'git'; ref: string; target: string; type: string; url: string; } | { content: string; output: 'content'; target: string; type: string; path?: string; }`\n\n### Example\n\n```typescript\nimport Stainless from '@stainless-api/sdk';\n\nconst client = new Stainless();\n\nconst targetOutput = await client.builds.targetOutputs.retrieve({\n  build_id: 'build_id',\n  target: 'node',\n  type: 'source',\n});\n\nconsole.log(targetOutput);\n```",
    perLanguage: {
      typescript: {
