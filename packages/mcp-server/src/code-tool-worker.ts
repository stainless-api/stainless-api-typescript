...
import { Stainless14, ClientOptions } from '@stainless-api/sdk';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Stainless14)`
    : `const run: (${functionSource.client}: Stainless14) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
