...
import { Stainless12, ClientOptions } from '@stainless-api/sdk';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Stainless12)`
    : `const run: (${functionSource.client}: Stainless12) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
